...
const ENDPOINT = 'https://get.dgc.gov.it/v1/dgc';
const OUTFILE = 'settings.json';

async function main() {
  const certificates = await loadKeys();
  const settings = await loadSettings();
  const contents = `${JSON.stringify({ settings, certificates }, null, '\t')}\n`;
  await fs.promises.writeFile(OUTFILE, contents);

  console.log(`Wrote settings & certificates to ${OUTFILE}`);
}
...
async function loadCertificates(resumeToken, keys) {
  const resp = await fetch(`${ENDPOINT}/signercertificate/update`, {
...
async function loadKeys() {
  ...
  const resp = await fetch(`${ENDPOINT}/signercertificate/status`);
